...
```bash
# 生成 .moon 文件
sudo zerotier-tool -genmoon /var/lib/zerotier-one/moon.json
```

此命令会在 `/var/lib/zerotier-one/` 目录下生成一个名为 `0000007axxxxxxxxxx.moon` 的文件。
...
1. 安装 ZeroTier 官方应用
2. 连接到你的网络
3. 目前移动客户端不支持手动添加 Moon 服务器，需要等待官方更新或通过其他方式配置
...
```bash
# 添加定时任务
echo "* * * * * root /path/to/moon-monitor.sh" | sudo tee /etc/cron.d/moon-monitor
```
